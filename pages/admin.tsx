...
export default function Admin() {
  ...
  React.useEffect(() => {
    async function loadClient() {
      console.log('[admin] loadClient called');
      console.log('[admin] window.supabase:', !!(window as any).supabase);
      if (!(window as any).supabase) {
        console.log('[admin] Supabase CDN not loaded yet, waiting...');
        return;
      }
      ...
      try {
        ...
        console.log('[admin] profiles result:', { count: profiles?.length, error: profErr });
        if (profErr) {
        ...
        try {
          const quiz = await fetch('/data/quizzes/v1.json').then(r => r.json()).catch(() => null);
